...
Using the NSW Transport OpenData API to obtain the list of permanent stations from around NSW (limited to 50 stations for this example)
...
Using the NSW Transport OpenData API to obtain the list of vehicle counts per hour from each permanent station (limited to 50 stations for this example)
...
Here we need to limit the LGA API pulls to just a few relevant LGA's so that it can pull quickly but still get the relevant information - left as eval=FALSE until we do this

The following code accesses an API with ABS to obtain demographic information based on Local Government Areas (LGA)
```{r,eval=FALSE}
...
This information does not contain the names of the LGA so this must also be sourced from the metadata
```{r,eval=FALSE}
# Names
...
And then joined to the original and cleaned
...
This data is then added to the Count data
...
traffic_vol_test<- traffic_vol_small[seq(1, 240, by = 24),]
